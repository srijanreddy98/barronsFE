...
class Collector {
    constructor(options) {
        ...
        this.messageRouter = new MessageRouter_1.MessageRouter(this.workingPackage.packageFolder, options.messageCallback, options.extractorConfig.messages || {});
        this.program = options.program;
        this.typeChecker = options.program.getTypeChecker();
        this._tsdocParser = new tsdoc.TSDocParser(api_extractor_model_1.AedocDefinitions.tsdocConfiguration);
        this.astSymbolTable = new AstSymbolTable_1.AstSymbolTable(this.program, this.typeChecker, this.packageJsonLookup, this.messageRouter);
    ...
    _fetchSymbolMetadata(astSymbol) {
        ...
        let effectiveReleaseTag = api_extractor_model_1.ReleaseTag.None;
        for (const astDeclaration of astSymbol.astDeclarations) {
            ...
            if (declaredReleaseTag !== api_extractor_model_1.ReleaseTag.None) {
                if (effectiveReleaseTag !== api_extractor_model_1.ReleaseTag.None && effectiveReleaseTag !== declaredReleaseTag) {
                    if (!astSymbol.isExternal) { // for now, don't report errors for external code
                        this.messageRouter.addAnalyzerIssue("ae-different-release-tags" /* DifferentReleaseTags */, 'This symbol has another declaration with a different release tag', astDeclaration);
                    }
                }
                else {
        ...
        if (effectiveReleaseTag === api_extractor_model_1.ReleaseTag.None && astSymbol.parentAstSymbol) {
            if (parentSymbolMetadata) {
                effectiveReleaseTag = parentSymbolMetadata.releaseTag;
            }
        }
        if (effectiveReleaseTag === api_extractor_model_1.ReleaseTag.None) {
            if (!astSymbol.isExternal) { // for now, don't report errors for external code
            ...
            effectiveReleaseTag = api_extractor_model_1.ReleaseTag.Public;
        }
