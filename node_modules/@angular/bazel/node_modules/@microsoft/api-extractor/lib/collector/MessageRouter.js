...
class MessageRouter {
    constructor(workingPackageFolder, messageCallback, messagesConfig) {
        // Normalized representation of the routing rules from api-extractor.json
        this._reportingRuleByMessageId = new Map();
        this._compilerDefaultRule = { logLevel: "none" /* None */,
            addToApiReportFile: false };
        this._extractorDefaultRule = { logLevel: "none" /* None */,
            addToApiReportFile: false };
        this._tsdocDefaultRule = { logLevel: "none" /* None */,
            addToApiReportFile: false };
        this.errorCount = 0;
        this.warningCount = 0;
    ...
    _applyMessagesConfig(messagesConfig) {
        ...
        if (messagesConfig.tsdocMessageReporting) {
            for (const messageId of Object.getOwnPropertyNames(messagesConfig.tsdocMessageReporting)) {
                ...
                else if (!api_extractor_model_1.AedocDefinitions.tsdocConfiguration.isKnownMessageId(messageId)) {
                    throw new Error(`Error in API Extractor config: The messages.tsdocMessageReporting table contains`
                        + ` an unrecognized identifier "${messageId}".  Is it spelled correctly?`);
                }
                else {
    ...
    addCompilerDiagnostic(diagnostic) {
        ...
        const messageText = TypeScriptMessageFormatter_1.TypeScriptMessageFormatter.format(diagnostic.messageText);
        const options = {
