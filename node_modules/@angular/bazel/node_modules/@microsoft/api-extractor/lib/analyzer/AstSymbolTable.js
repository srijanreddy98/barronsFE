...
class AstSymbolTable {
    constructor(program, typeChecker, packageJsonLookup, messageRouter) {
        /**
        ...
        // for that node.
        this._entitiesByIdentifierNode = new Map();
        ...
        this._exportAnalyzer = new ExportAnalyzer_1.ExportAnalyzer(this._program, this._typeChecker, {
            ...
            case ts.SyntaxKind.TypeQuery: // represents for "typeof X" as a type
                {
                    ...
                    if (identifierNode) {
                        let referencedAstEntity = this._entitiesByIdentifierNode.get(identifierNode);
    ...
    _fetchEntityForIdentifierNode(identifierNode, governingAstDeclaration) {
        let referencedAstEntity = this._entitiesByIdentifierNode.get(identifierNode);
        if (!referencedAstEntity) {
            const symbol = this._typeChecker.getSymbolAtLocation(identifierNode);
            if (!symbol) {
    ...
    _fetchAstSymbol(options) {
        ...
        if (!TypeScriptHelpers_1.TypeScriptHelpers.hasAnyDeclarations(followedSymbol)) {
            return undefined;
        }
        ...
        if (followedSymbol.flags & (ts.SymbolFlags.TypeParameter | ts.SymbolFlags.TypeLiteral | ts.SymbolFlags.Transient)
            && !TypeScriptInternals_1.TypeScriptInternals.isLateBoundSymbol(followedSymbol)) {
            return undefined;
        }
        ...
        if (!astSymbol) {
            ...
            if (!nominalAnalysis) {
                ...
                if (arbitraryParentDeclaration) {
                    ...
                    if (!parentAstSymbol) {
                        throw new node_core_library_1.InternalError('Unable to construct a parent AstSymbol for '
                            + followedSymbol.name);
                    }
                }
            }
            ...
            if (localName === undefined) {
                ...
                if (TypeScriptHelpers_1.TypeScriptHelpers.isWellKnownSymbolName(localName)) {
                    ...
                    const isUniqueSymbol = TypeScriptHelpers_1.TypeScriptHelpers.isUniqueSymbolName(localName);
                    for (const declaration of followedSymbol.declarations || []) {
