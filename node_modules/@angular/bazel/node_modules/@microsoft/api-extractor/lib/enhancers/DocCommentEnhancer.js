...
class DocCommentEnhancer {
    ...
    _analyzeDeclaration(astDeclaration) {
        const metadata = this._collector.fetchMetadata(astDeclaration);
        ...
        if (metadata.docCommentEnhancerVisitorState === VisitorState_1.VisitorState.Visiting) {
            this._collector.messageRouter.addAnalyzerIssue("ae-cyclic-inherit-doc" /* CyclicInheritDoc */, `The @inheritDoc tag for "${astDeclaration.astSymbol.localName}" refers to its own declaration`, astDeclaration);
            return;
        }
    ...
    _analyzeNeedsDocumentation(astDeclaration, metadata) {
        if (astDeclaration.declaration.kind === ts.SyntaxKind.Constructor) {
            ...
            const configuration = api_extractor_model_1.AedocDefinitions.tsdocConfiguration;
            ...
            const symbolMetadata = this._collector.fetchMetadata(astDeclaration.astSymbol);
            if (symbolMetadata.releaseTag === api_extractor_model_1.ReleaseTag.Internal) {
                // If the constructor is marked as internal, then add a boilerplate notice for the containing class
                const classMetadata = this._collector.fetchMetadata(classDeclaration);
        ...
        if (node instanceof tsdoc.DocLinkTag) {
            if (node.codeDestination) {
                ...
                if (node.codeDestination.packageName === undefined
                    ...
                    const referencedAstDeclaration = this._collector.astReferenceResolver
    ...
    _applyInheritDoc(astDeclaration, docComment, inheritDocTag) {
        ...
        const referencedMetadata = this._collector.fetchMetadata(referencedAstDeclaration);
